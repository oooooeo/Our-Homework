...
- 帮助与反馈：员工端提供反馈入口，可提交满意、不满意、建议，也可提交可公共检索的问题。
- 公开问题库：员工可搜索历史问题和后台回复，后台可在公开问题入口统一回复并标记解决。
...
如果需要启用“公开问题库”和“后台回复问题”，需要在 Supabase 的 SQL Editor 中执行：

```text
training-web/supabase-migration-20260611-public-questions.sql
```

执行完成后，刷新总入口、员工端和后台端，密码登录、撤回任务、部门登记、注销员工、定向发布、学习广场、员工反馈和公开问题库会启用。
