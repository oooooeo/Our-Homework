...
- 入口登录：用户在总入口设置唯一用户名后进入员工端，用户名 `supermanager` 进入后台端。
...
如果首次启用用户名注册登录，需要在 Supabase 的 SQL Editor 中执行：

```text
training-web/supabase-migration-20260609-username-auth.sql
```

执行完成后，刷新员工端和后台端，截止时间和评论区会启用。

当前仍是轻量版本，用户名保存在浏览器本地，后台入口只做前端识别。正式用于培训时，建议继续接入 Supabase Auth，让员工使用密码或验证码登录。
